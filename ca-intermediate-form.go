...
func showCAIntForm(app *qt.QApplication, window *qt.QMainWindow) *qt.QFormLayout {
	config := ConfigParser()
	formLayout := qt.NewQFormLayout(nil)
	formLayout.SetFieldGrowthPolicy(qt.QFormLayout__ExpandingFieldsGrow)
	label := qt.NewQLabel3("Create New Intermediate Certificate Authority")
	...
	CName.SetPlaceholderText("intermediate.qt.com")
	...
	formLayout.InsertRow6(9, optionGroup.QLayout)
	...
	addButton.OnClicked(func() {
		if len(CName.Text()) >= 1 && len(CN.Text()) >= 1 {
			...
			qt.QMessageBox_Critical(nil, "error", "You must supply a common name and a Intermediate CA name")
		}
	})
